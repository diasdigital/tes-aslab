                ...
                <div class="row gx-4 gx-lg-5 justify-content-center">
                    <div class="col-md-10 col-lg-8 col-xl-7">
                        <div class="post-heading">
                            <h1>{{ $data->judul }}</h1>
                            <h2 class="subheading">{{ $data->subjudul }}</h2>
                            <span class="meta">
                                {{ $data->kategori }}
                            </span>
                        </div>
        ...
        <article class="mb-4">
            <div class="container px-4 px-lg-5">
                <div class="row gx-4 gx-lg-5 justify-content-center">
                    <div class="col-md-10 col-lg-8 col-xl-7">
                        <p>{{ $data->deskripsi }}</p>
                        <p>Deadline TP: {{ $data->deadline }}</p>

                        <a class="btn btn-primary" href="/edit/{{ $data->id }}">Edit TP</a>
                        <form action="/delete/{{ $data->id }}" method="POST" class="d-inline">
                            @method('delete')
                            @csrf
                            <button class="btn btn-danger text-white" onclick="return confirm('Apa kamu yakin ingin menghapus TP {{ $data->judul }}?')">Hapus TP</button>
                          </form>
                    </div>
                </div>
            </div>
